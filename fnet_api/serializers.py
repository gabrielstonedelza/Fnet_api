...
class CashDepositSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CashDeposit
        fields = ['id', 'guarantor', 'customer', 'agent', 'guarantor_username', 'customer_username', 'deposit_paid', 'agent_username', 'amount', 'request_status', 'date_requested', 'time_requested','slug']
        read_only_fields = ['agent']
...
class BankDepositSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BankDeposit
        fields = ['id', 'guarantor', 'customer', 'agent', 'guarantor_username', 'customer_username','deposit_paid', 'agent_username','bank','account_number','account_name','amount', 'request_status', 'date_requested', 'time_requested','slug']
        read_only_fields = ['agent']
...
class PaymentsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MyPayments
        fields = ['id', 'agent', 'agent_username', 'mode_of_payment1','mode_of_payment2', 'cash_at_location1','cash_at_location2', 'amount','amount1','amount2','bank1','bank2', 'transaction_id1','transaction_id2', 'payment_action','payment_status', 'date_created', 'time_created','slug']
        read_only_fields = ['agent']
...
class MobileMoneyWithdrawalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id','agent','username','customer_phone','customer_name','network','type','id_type','id_number','amount','charges','agent_commission','cash_out_commission','mtn_commission','date_of_withdrawal','time_of_withdrawal']
        read_only_fields = ['agent']
...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Notifications
        fields = ['id','item_id','notification_title','notification_message','read','customer','user','user2','customer_request_slug','cash_deposit_request_slug','bank_deposit_request_slug','payment_slug','date_created','slug']
